...
1. **DBX 供应商交互复现** — 设置中心 `ai-providers` 为 list/edit、品牌下拉（合作方空列）、拍平字段；表单候选项按内容定宽。过程 owner：[1616-dbx-provider-parity/task-card.md](260907/1616-dbx-provider-parity/task-card.md)。已合入本机 `czz-dev`；上游 [#1178](https://github.com/Syngnat/GoNavi/pull/1178)（压平 `feat/ai-provider-list-edit`）。r63 实机待确认。
2. **供应商设置页第十六轮（编辑收缩）** — r59 实机已通过；上游 [#1155](https://github.com/Syngnat/GoNavi/pull/1155) MERGED。该页主交互已被上一行替换。
...
| 03 | [03-260903-squash-编辑收缩展示](../pr/03-260903-squash-编辑收缩展示.md) | [#1155](https://github.com/Syngnat/GoNavi/pull/1155) MERGED |

## 未验 / 未开

- 已接入芯片行拖拽排序仍未做。
- 真实模型回复、Windows/Linux 实机、签名发布包。
- [#1178](https://github.com/Syngnat/GoNavi/pull/1178) 待上游评审；`vibe/` 与 `czz-docs/` 未进该 PR。
