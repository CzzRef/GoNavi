...
> 当前路径：`vibe/specs/260901/0000-ai-provider-management/task-card.md`。旧 `czz-docs/` 同名文件只是归档桩。上游 PR 史：[01](../../../knowledge/upstream-pr/01-260901-squash-供应商目录与CLI发现.md) · [02](../../../knowledge/upstream-pr/02-260902-followup-设置页折叠与CLI续命.md) · [03](../../../knowledge/upstream-pr/03-260903-pending-编辑收缩展示.md)。第十六轮细节在 [编辑收缩卡](../../260903/0000-provider-editor-compact/task-card.md)。
...
- 已隐藏是目录底栏抽屉：有隐藏项才出现，收起占一行，展开后列表自滚并封顶。有隐藏项时顶部分隔条可拖高钉住（双击复位）；未钉住不占固定高度。目录卡片区内部滚动、只拖宽。左键点击隐藏项与目录相同：未接入则添加，已接入 CLI 则编辑；眼睛仍只负责恢复显示。搜索提示隐藏结果但不自动展开，全部隐藏时仍保留恢复入口。隐藏/恢复不触发保存、删除、检查或默认切换。
...
布局偏好（含隐藏候选）只存于本机 `gonavi.ai.providers.layout.v1`，不写入供应商配置或凭证；清除该偏好会恢复完整目录，不跨设备同步。独立交互预览已改为挂载正式 React 组件（`AISettingsProvidersSection` + 侧栏 + 同一套布局/保存工具函数），Wails 桥接为内存模拟。追踪目录仍为 `gonavi-provider-preview-r32`，以免分享短链失效。仍是模拟数据，不用预览结果证明正式产品可用性。
...
2026-09-01 的实机反馈集中在同一张编辑页，改动彼此咬合，因此实现上并为一批提交。

说明文案原本每条独占一行，配置页一矮就把表单挤出视野。现改为收进标题旁的图标；图标内保留一份视觉隐藏文本，读屏与静态渲染仍可命中，不是简单删掉。报错后的自动定位改为只移动编辑区自己的 `scrollTop`——`scrollIntoView` 会连带滚动 `overflow: hidden` 的祖先，而那些祖先没有滚动条可以滚回来，表现为设置页顶栏被永久顶掉。

保存与另存为合成一个按钮，抹平 `Space.Compact` 留下的沙漏缺口，右侧换成向下箭头；单例 CLI 预设复用同一份本机登录，直接不渲染下拉而不是渲染后置灰。另存为的命名同轮改为按重名判定：用户改过名字且不撞名时原样保留，只有撞名才追加后缀。
...
本轮把操作口径与界面取舍写成两份规则正文：[gonavi-verify-build-restart.md](../../../knowledge/gonavi-verify-build-restart.md) 管产物轮次、打包四键与停旧起新；[ai-provider-ui-conventions.md](../../../knowledge/ai-provider-ui-conventions.md) 管悬浮提示、滚动定位、弹窗几何、破坏性操作、渲染性能、断点与命名、表单折叠行等约定。前者同时升级为 CodeNote 的 `project` 作用域 Skill `gonavi-verify-build-restart`，文档与 Skill 互为投影，改动时两边同改。
...
r55 修正（2026-09-03，按 [260903 task card §11](../../260903/0000-provider-editor-compact/task-card.md#11-r55-核验与修正2026-09-03)）：「够宽时可同一行」从 ⓘ 正文移进 `?`；默认模型标题旁去掉长文；来源只留左 ⓘ，右 ⓘ 仅 CLI 且并入原空折叠的命令/能力失败/effort 未校验；点选择器本体不再带「启用管理」外壳；`is-default` 只标默认行；并排 URL 悬浮全文；模型目录回退限同一编辑会话；清掉 effort 死 prop 与孤儿 CSS。`tsc` 通过；全量 vitest 5144 项中 5142 通过，两条失败均为已知基线（`main.browserMock`、`testPolicy`）。
...
r57 修正（2026-09-03，按 [260903 task card §13](../../260903/0000-provider-editor-compact/task-card.md#13-r57-实机反馈修正2026-09-03)）：拖拽副本 portal 到 `<body>` 修掉弹窗 `transform` 造成的鼠标错位，副本微倾半透明贴抓取点；可拖拽面悬浮抓手光标、拖动中全局 `grabbing`；模型启用管理的「已停用」等反馈改为该行开关旁 1.6 秒小浮窗，底部说明行删除，停用行整行置灰。`tsc` 通过；全量 vitest 5154 项中 5152 通过（+1 为本轮新增用例），两条失败仍为已知基线。
...
r58 修正（2026-09-03，按 [260903 task card §14](../../260903/0000-provider-editor-compact/task-card.md#14-r58-并排阈值2026-09-03)）：并排换行阈值按压缩后文字重定（格式 140 / URL 200 / Key 150px，约 520px 起三项同排），失焦字段比压缩文字窄时省略号。仅 CSS。

本轮核验包 r58：`build/bin/GoNavi-provider-settings-260903-r58`（100547186 字节，SHA-256 `698ac52eef253be6a2f6ddfc907e90abf901a8dde567c824044add59d930ce71`），壳为 `GoNavi-Provider-Verification-r58.app`。已被 r59 替换。

r59 重建（2026-09-03）：合入最新 `upstream/dev`（`a70650b3`，含 [#1134](https://github.com/Syngnat/GoNavi/pull/1134) 与 harness）后源码晚于 r58，重打核验包。`tsc` 通过；全量 vitest 5144/5144（上游已消掉原先两条基线失败；本地工具用例随 harness 迁移减少）。编译与离线测试不构成实机通过。

本轮核验包 r59：`build/bin/GoNavi-provider-settings-260903-r59`（101861154 字节，SHA-256 `8e000301579a9f6f1bf505832414726a24831d2a552c6225a3ad17545e4f283e`），壳为 `GoNavi-Provider-Verification-r59.app`。进程回读 pid 62193，仅此一例。

**实机结果（2026-09-03）**：用户确认 r59 实验通过（并排压缩、布局分段钮、单选项只读、CLI 缓存/空折叠、目录拖拽，以及 r57/r58 修正）。换 identifier 会清空 WebView 本地存储。已接入芯片行的拖拽排序仍未做。
...
| 项目 | 本轮结果 | 证据边界 |
| --- | --- | --- |
| 前端全量回归（r29） | 5035/5037，549 文件 | 通过数较第十二轮增 12，等于本轮新增用例数；两条失败未变 |
| 第十三轮实机核验，r29/r30 | 通过 | 提示图标、报错定位只滚编辑区、保存下拉、卡片删除角标、收展图标、行距与底部单行、副本命名，七项均由用户实机确认 |
...
| 构建与重启 | 通过至 r48 | 第十二轮至 r19，第十三轮至 r29，第十四轮至 r48；均为 Vite 生产包 + `wails build -s -skipbindings -nosyncgomod`。r48 回读单实例。独立预览已按 r48 源码重同步 |
| 独立预览崩溃（2026-09-02） | 已重建同目录 `index.html` | 内联器用 `String.replace` 把 React 的 `$&` 展开成 `<script src>`，包内 `</script>` 截断页面。改为切片插入 + 转义；`node --check` 通过；本机分享 `Content-Length` 7415803。页面短链为 `https://s.comeon365.com/fD3YJ82ww`（`index.html`）。`https://s.comeon365.com/fzGVLe__E` 是目录「最新」别名，改 README 后曾指向说明文件，已 `touch index.html` 拨回。需硬刷新。不是桌面核验包变更 |
...
当前产物为 `build/bin/GoNavi-provider-settings-260901-r48`，100,331,970 字节，SHA-256 `f1b934947374fe1ec3e7d0a52a5d933de20dbe343dd9a1f7d5f1469a094b9d60`。原生核验使用独立 `GoNavi-Provider-Verification-r48.app`；未覆盖安装版，也不是签名发布包。r8–r47 及早期产物保留。r48 对齐已隐藏底栏拖高、三框 14.5em 与提示不截鼠标。独立预览 `gonavi-provider-preview-r32` 已按 r48 源码重同步。
...
当前核验包为 r48：`GoNavi-Provider-Verification-r48.app` / `GoNavi-provider-settings-260901-r48`，100,331,970 字节，SHA-256 `f1b934947374fe1ec3e7d0a52a5d933de20dbe343dd9a1f7d5f1469a094b9d60`。菜单名与 Dock 名均为 `GoNavi Provider Verification r48`。
...
```json documentation-sync-group-v1
{
  ...
  "group_key": "dsg:gonavi:ai-provider-management",
  "group_owner": "vibe/specs/260901/0000-ai-provider-management/task-card.md",
  "documents": [
