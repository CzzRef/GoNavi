...
Tool: grok / DSH
Date: 2026-09-03
Status: `implemented-r58`（r57 之后并排宽度阈值调整，r58 已按 §14 修正，等实机核验）
Product: GoNavi AI 设置 · 模型供应商
Predecessor: [第十五轮认证字段默认纵向](ai-provider-management-task-card.md#第十五轮认证字段默认纵向) · [界面约定](ai-provider-ui-conventions.md)

> 用户要求先核验说明，再写 task card 供本人核验。通过前不实施。

## 1. 核验对照（现状 vs 你的五条）

对照源码：`frontend/src/components/ai/AISettingsProvidersSection.tsx`、`AIProviderModelSelect.tsx`、`ai-provider-ui-conventions.md` §A/B/D/G。
...
- In scope: 供应商编辑页连接字段、布局 ⓘ、默认模型弹层、目录 CLI 提示；`ai-provider-ui-conventions.md` 增补；CodeNote 侧给「分段选择 + 旁置问号」一条偏好指针（不复制像素）。
...
| 文档 | 本卡通过后写入 `ai-provider-ui-conventions.md` 新节；管理总卡加第十六轮指针；CodeNote `contextual-help` 只加「分段选择旁置问号」邻居句 |
...
9. CLI 编辑页不再出现空的「本机 CLI … >」折叠条；提示在芯片右上角或一行短文+ⓘ（按你选的方案）。
...
r58 核验包：`build/bin/GoNavi-provider-settings-260903-r58`（100547186 字节，SHA-256 `698ac52eef253be6a2f6ddfc907e90abf901a8dde567c824044add59d930ce71`），壳 `GoNavi-Provider-Verification-r58.app`，pid 52168 单实例运行中。identifier 又换了：`connectionLayout` 回到默认纵向，需在 `?` 里重新点「并排」，再把编辑区拉到 520px 上下看三项何时同排。r57 三条（贴鼠标 / 抓手 / 行内浮窗）在 r58 上一并看。
