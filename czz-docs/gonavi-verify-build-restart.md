...
本文件是本仓库「改一轮 → 出一个独立核验程序 → 停旧起新」这条通路的规则正文。它不随上游 PR 提交（见 [upstream-pr-scope.md](upstream-pr-scope.md)）。
...
同轮更新 [ai-provider-management-task-card.md](ai-provider-management-task-card.md) 的验收产物段：产物路径、字节数、SHA-256、核验包名。旧轮产物保留，不删。
...
全量 `npx vitest run` 有两条**长期失败**，与本仓库改动无关，不得计为回归：

| 用例 | 性质 |
| --- | --- |
| `src/main.browserMock.test.ts > localizes browser mock provider test messages` | 合并上游前即已复现，预存在 |
| `src/testPolicy.test.ts > adds no new test that reads source files from disk` | 上游自带：`App.collapsed-titlebar-actions.test.ts` 与 `components/TitleBarPrimaryActions.test.tsx` 违反上游自己的守卫，却未登记进基线名单 |
