...
状态：正式页面已落实预览验收后的管理交互和隐藏目录。2026-09-01 第十二轮合并上游 `dev` 并修掉窄屏遮罩、弹窗缩放与高度跳动；第十三轮把编辑页的提示、定位与操作入口整体收敛，并把本轮沉淀的操作与界面规则落成两份正文加一个 project Skill。全量前端 5037 项中 5035 通过，两条失败分别为既有基线与上游自带。用户已在 r18 确认弹窗不再跳动；**第十三轮的实机观感项一项都未验收**，不能把编译和离线测试当作这些项目通过。
...
- 新增使用可搜索下拉，不再前置选择端点。页内不再重复左侧「模型供应商」标题；说明与添加下拉同一行且左对齐，添加框只比文案略宽。供应商目录可折叠，默认三列，可拖宽、独立滚动并填满可用高度；卡片高度随内容，长名称单行省略、悬浮看全文；隐藏按钮紧贴图标右侧并缩小，不再压在名称上；已接入 CLI 用绿色对勾，悬浮显示「已启用」，不再用「已接入」文案。查找框与「供应商目录」同一行，收起目录时隐藏。窄窗口改用抽屉，编辑表单随宽度组合字段。设置中心进入 AI 页时保持与其他设置页相同的 1080×820 外壳，不因进入 AI 而自行放大。
...
| 第十三轮实机核验 | **未开始** | 提示图标、报错定位、保存下拉、卡片删除角标、收展图标、行距与底部单行、副本命名，七项均未由用户确认 |
...
当前产物为 `build/bin/GoNavi-provider-settings-260901-r29`，100,298,866 字节，SHA-256 `6da60aa63b4aec8fc2be134b083195ed8e0d93d2647da40630092167c4f8480e`。原生核验使用独立 `GoNavi-Provider-Verification-r29.app`；未覆盖安装版，也不是签名发布包。r8–r28 及早期产物保留。**它是当前产物，不是已验收产物**——第十三轮的观感项尚未由用户确认。
...
当前核验包为 r29：`GoNavi-Provider-Verification-r29.app` / `GoNavi-provider-settings-260901-r29`，100,298,866 字节，SHA-256 `6da60aa63b4aec8fc2be134b083195ed8e0d93d2647da40630092167c4f8480e`。菜单名与 Dock 名均为 `GoNavi Provider Verification r29`，不再只写在 DisplayName 里。
