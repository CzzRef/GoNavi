...
状态：正式页面已落实预览验收后的管理交互和隐藏目录。2026-08-31 第十轮修复原生核验发现的草稿误判，前端 211 项、TypeScript、后端两组竞态回归及六语言检查通过；独立 r10 macOS 程序已启动。当前系统锁屏，r10 原生补验与真实 CLI 检查尚未完成，不能将编译和离线测试作为这些项目通过的依据。

工作分支为 `czz-dev`，实施起点为 `4cc7493c`。本轮获得重启、Computer Use 核验、优化、提交和推送授权；交付目标是新建 `origin/czz-dev`，不合并或覆盖 `origin/dev`。提交结果以 Git 历史和远端引用为准。
...
- 新增使用可搜索下拉，不再前置选择端点。供应商目录可折叠，默认三列，可拖宽、独立滚动并填满可用高度；窄窗口改用抽屉，编辑表单随宽度组合字段。
...
- 隐藏分组固定在目录底部，展开列表独立滚动；搜索提示隐藏结果但不自动展开，全部隐藏时仍保留恢复入口。隐藏/恢复不触发保存、删除、检查或默认切换。
...
| 构建与重启 | 通过 | r10 darwin/arm64 构建 44.133 秒，跳过绑定再生成和 go.mod 同步；以无 nvm 的 GUI PATH 启动独立测试程序，进程身份已回读 |
...
验收产物为 `build/bin/GoNavi-provider-settings-260831-r10`，99,901,970 字节，SHA-256 `7bbf77dd7395f6a8a20fbf82005c76e5ef363bb0dda8bf2ce7d0ff91abf73d22`。原生核验使用独立 `GoNavi-Provider-Verification-r10.app`，未覆盖安装版，也不是签名发布包。r8/r9 及早期产物保留。
