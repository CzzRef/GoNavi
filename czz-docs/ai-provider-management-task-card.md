...
上游 PR 的范围、排除项、测试口径与拆分障碍单独记在 [upstream-pr-scope.md](upstream-pr-scope.md)。[#1131](https://github.com/Syngnat/GoNavi/pull/1131) 已合入 `dev`。跟进 [#1134](https://github.com/Syngnat/GoNavi/pull/1134) 只含设置页与 CLI 空闲续命；例程闸门已回滚，不进该 PR。
...
| 独立预览崩溃（2026-09-02） | 已重建同目录 `index.html` | 内联器用 `String.replace` 把 React 的 `$&` 展开成 `<script src>`，包内 `</script>` 截断页面。改为切片插入 + 转义；`node --check` 通过；本机分享 `Content-Length` 7415803。短链仍为 `https://s.comeon365.com/fzGVLe__E`，需硬刷新。不是桌面核验包变更 |
